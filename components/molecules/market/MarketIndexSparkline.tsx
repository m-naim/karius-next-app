...
const historyCache = new Map<string, { value: number; date: string }[]>()

export function MarketIndexSparkline({ symbol, color = '#3b82f6', period = '1y' }: MarketIndexSparklineProps) {
  const cacheKey = `${symbol}_${period}`
  const [data, setData] = useState<{ value: number; date: string }[]>(historyCache.get(cacheKey) || [])
